...
def test_tile_data_stores_type_road_height_and_coords():
    data = big_tile_data.BigTileData(tile_type=3, has_road=True, height=42, x=5, y=10)
...
def test_creation_and_get_tile_type():
    data = big_tile_data.BigTileData(tile_type=3, has_road=True, height=42, x=0, y=0)
    assert data.tile_type == 3

@pytest.mark.parametrize("tile_type", [0, 1, 7, 15])
def test_tile_data_accepts_valid_tile_type_range(tile_type):
    data = big_tile_data.BigTileData(tile_type=tile_type, has_road=False, height=0, x=0, y=0)
...
def test_tile_data_rejects_invalid_tile_type_range(tile_type):
    with pytest.raises(ValueError):
        big_tile_data.BigTileData(tile_type=tile_type, has_road=False, height=0, x=0, y=0)
...
def test_tile_data_rejects_non_int_tile_type_values(invalid_value):
    with pytest.raises(TypeError):
        big_tile_data.BigTileData(tile_type=invalid_value, has_road=False, height=0, x=0, y=0)
...
def test_tile_data_accepts_valid_road_values(has_road):
    data = big_tile_data.BigTileData(tile_type=1, has_road=has_road, height=0, x=0, y=0)
...
def test_tile_data_rejects_non_bool_road_values(invalid_value):
    with pytest.raises(TypeError):
        big_tile_data.BigTileData(tile_type=1, has_road=invalid_value, height=0, x=0, y=0)
...
def test_tile_data_accepts_valid_height_range(height):
    data = big_tile_data.BigTileData(tile_type=1, has_road=False, height=height, x=0, y=0)
    assert data.height == height

@pytest.mark.parametrize("height", [-1, 256, 999])
def test_tile_data_rejects_invalid_height_range(height):
    with pytest.raises(ValueError):
        big_tile_data.BigTileData(tile_type=1, has_road=False, height=height, x=0, y=0)

@pytest.mark.parametrize("invalid_value", ["1", None, 1.5, True])
def test_tile_data_rejects_non_int_height_values(invalid_value):
    with pytest.raises(TypeError):
        big_tile_data.BigTileData(tile_type=1, has_road=False, height=invalid_value, x=0, y=0)
...
def test_tile_data_accepts_valid_x_coords(x):
    data = big_tile_data.BigTileData(tile_type=1, has_road=False, height=0, x=x, y=0)
    assert data.x == x

@pytest.mark.parametrize("y", [0, 1, 10, 100])
def test_tile_data_accepts_valid_y_coords(y):
    data = big_tile_data.BigTileData(tile_type=1, has_road=False, height=0, x=0, y=y)
    assert data.y == y

@pytest.mark.parametrize("invalid_value", ["1", None, 1.5, True])
def test_tile_data_rejects_invalid_type_coords(invalid_value):
    with pytest.raises(TypeError):
        big_tile_data.BigTileData(tile_type=1, has_road=False, height=0, x=invalid_value, y=0)
    with pytest.raises(TypeError):
        big_tile_data.BigTileData(tile_type=1, has_road=False, height=0, x=0, y=invalid_value)
